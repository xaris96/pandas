...
class CommonDBTests(unittest.TestCase):
    ...
    def helpTestDataType(
        ...
        if DBAPIDataTypeString == "STRING":
            assert descTuple[1] == api.STRING, 'was "{}" expected "{}"'.format(
                descTuple[1],
                api.STRING.values,
            )
        elif DBAPIDataTypeString == "NUMBER":
            assert descTuple[1] == api.NUMBER, 'was "{}" expected "{}"'.format(
                descTuple[1],
                api.NUMBER.values,
            )
        elif DBAPIDataTypeString == "BINARY":
            assert descTuple[1] == api.BINARY, 'was "{}" expected "{}"'.format(
                descTuple[1],
                api.BINARY.values,
            )
        elif DBAPIDataTypeString == "DATETIME":
            assert descTuple[1] == api.DATETIME, 'was "{}" expected "{}"'.format(
                descTuple[1],
                api.DATETIME.values,
            )
        elif DBAPIDataTypeString == "ROWID":
            assert descTuple[1] == api.ROWID, 'was "{}" expected "{}"'.format(
                descTuple[1],
                api.ROWID.values,
            )
        elif DBAPIDataTypeString == "UUID":
            assert descTuple[1] == api.OTHER, 'was "{}" expected "{}"'.format(
                descTuple[1],
                api.OTHER.values,
            )
        else:
        ...
        for inParam in inputs:
            ...
            if allowedReturnValues:
                ...
                assert isinstance(rs[0], allowedTypes), (
                    'result type "{}" must be one of {}'.format(type(rs[0]), allowedTypes)
                )
            else:
            ...
            if compareAlmostEqual and DBAPIDataTypeString == "DATETIME":
                ...
                assert abs(v - s) / s < 0.00001, (
                    "Values not almost equal recvd={}, expected={:f}".format(rs[0], s)
                )
            else:
                if allowedReturnValues:
                    ok = False
        ...
        for inParam in inputs:
            ...
            for j in range(len(inParam)):
                assert rec[j] == inParam[j], (
                    'returned value:"{}" != test value:"{}"'.format(rec[j], inParam[j])
                )
    ...
    def testFormatParamstyle(self):
        ...
        for inParam in inputs:
            ...
            self.assertEqual(
                rec[0],
                inParam,
                'returned value:"{}" != test value:"{}"'.format(rec[0], inParam),
            )
        ...
        assert crsr.command == sel, 'expected:"{}" but found "{}"'.format(sel, crsr.command)
...
class TestADOwithSQLServer(CommonDBTests):
    ...
    def testMultipleSetReturn(self):
        ...
        spdef = """
            ...
            AS
                SELECT fldData FROM xx_{} ORDER BY fldData ASC
                SELECT fldData From xx_{} where fldData = -9999
                SELECT fldData FROM xx_{} ORDER BY fldData DESC
                    """.format(
            config.tmp,
            config.tmp,
            config.tmp,
...
if __name__ == "__main__":
    ...
    with cleanup_manager():
        ...
        print(__doc__)
        print("Default Date Converter is {}".format(defaultDateConverter))
