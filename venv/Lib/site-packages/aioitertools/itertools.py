...
async def groupby(
    itr: AnyIterable[T], key: KeyFunction[T, R] | None = None
...
@overload
def islice(
    itr: AnyIterable[T], __stop: int | None
...
@overload
def islice(
    itr: AnyIterable[T], __start: int, __stop: int | None, __step: int = 1
) -> AsyncIterator[T]:  # pragma: nocover
    pass


async def islice(itr: AnyIterable[T], *args: int | None) -> AsyncIterator[T]:
    """
...
async def permutations(
    itr: AnyIterable[T], r: int | None = None
) -> AsyncIterator[tuple[T, ...]]:
    """
