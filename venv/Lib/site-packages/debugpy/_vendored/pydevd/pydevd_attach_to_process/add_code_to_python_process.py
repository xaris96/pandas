...
def get_target_filename(is_target_process_64=None, prefix=None, extension=None):
    ...
    if arch.lower() not in ("amd64", "x86", "x86_64", "i386", "x86"):
        ...
        try:
            ...
            print("Error listing dir: {}".format(libdir))
            traceback.print_exc()
            return None
        ...
        if filename is None:
            print("Unable to attach to process in arch: {} (did not find {} in {}).".format(arch, expected_name, libdir))
            return None

        print("Using {} in arch: {}.".format(filename, arch))
        ...
        if not prefix:
            ...
            if IS_WINDOWS or IS_MAC:  # just the extension changes
                ...
                print("Unable to attach to process in platform: {}".format(sys.platform))
                return None

        filename = os.path.join(libdir, "{}{}{}".format(prefix, suffix, extension))

    if not os.path.exists(filename):
        print("Expected: {} to exist.".format(filename))
        return None
...
def run_python_code_windows(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    with _acquire_mutex("_pydevd_pid_attach_mutex_{}".format(pid), 10):
        print("--- Connecting to {} bits target (current process is: {}) ---".format(bits, 64 if is_python_64bit() else 32))
        sys.stdout.flush()
        ...
        with _win_write_to_shared_named_memory(python_code, pid):
            ...
            print("\n--- Injecting attach dll: {} into pid: {} ---".format(os.path.basename(target_dll), pid))
            sys.stdout.flush()
            ...
            with _create_win_event("_pydevd_pid_event_{}".format(pid)) as event:
                print("\n--- Injecting run code dll: {} into pid: {} ---".format(os.path.basename(target_dll_run_on_dllmain), pid))
                sys.stdout.flush()
...
def _win_write_to_shared_named_memory(python_code, pid):
    ...
    assert len(python_code) < BUFSIZE - 1, "Python code must have at most {} bytes (found: {})".format(BUFSIZE - 1, len(python_code))
    ...
    PAGE_READWRITE = 0x4
    FILE_MAP_WRITE = 0x2
    filemap = CreateFileMapping(INVALID_HANDLE_VALUE, 0, PAGE_READWRITE, 0, BUFSIZE, "__pydevd_pid_code_to_run__{}".format(pid))

    if filemap == INVALID_HANDLE_VALUE or filemap is None:
        raise Exception("Failed to create named file mapping (ctypes: CreateFileMapping): {}".format(filemap))
    try:
...
def run_python_code_linux(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    if gdb_load_shared_libraries:
        print("PYDEVD_GDB_SCAN_SHARED_LIBRARIES set: {}.".format(gdb_load_shared_libraries))
        cmd.extend(["--init-eval-command='set auto-solib-add off'"])  # Don't scan all libraries.

        for lib in gdb_load_shared_libraries.split(","):
            lib = lib.strip()
            cmd.extend(["--eval-command='sharedlibrary {}'".format(lib)])  # Scan the specified library
    else:
    ...
    cmd.extend(
        [
            ...
            "--eval-command='call (int)DoAttach({}, \"{}\", {})'".format(is_debug, python_code, show_debug_info),
        ]
    )
...
def run_python_code_mac(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    cmd.extend(
        [
            ...
            "-o 'command script import \"{}\"'".format(lldb_prepare_file),
            '-o \'load_lib_and_attach "{}" {} "{}" {}\''.format(target_dll, is_debug, python_code, show_debug_info),
        ]
    )
...
def test():
    print("Running with: {}".format(sys.executable))
    code = """
import os, time, sys
print(os.getpid())
