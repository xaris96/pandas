...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    def __init__(
        ...
        url: str | None = None,
        branch_path: PathLike | None = None,
    ) -> None:
    ...
    def __repr__(self) -> str:
        return "git.{}(name={}, path={}, url={}, branch_path={})".format(
            type(self).__name__,
            self._name,
            self.path,
            self.url,
    ...
    def _config_parser(
        ...
        if not repo.bare and parent_matches_head and repo.working_tree_dir:
            ...
            try:
                ...
                raise OSError(
                    "Could not find {} file in the tree of parent commit {}".format(cls.k_modules_file, parent_commit)
                ) from e
    ...
    def add(
        ...
        clone_multi_options: Sequence[TBD] | None = None,
        allow_unsafe_options: bool = False,
        allow_unsafe_protocols: bool = False,
    ) -> "Submodule":
        ...
        if has_module and url is not None:
            if url not in [r.url for r in sm.module().remotes]:
                raise ValueError(
                    "Specified URL '{}' does not match any remote url of the repository at '{}'".format(url, sm.abspath)
                )
            # END check url
        # END verify urls match

        mrepo: Repo | None = None
        ...
        if url is None:
            ...
            # Clone new repo.
            kwargs: dict[str, bool | int | str | Sequence[TBD]] = {"n": no_checkout}
        ...
        writer: GitConfigParser | SectionConstraint
        ...
        try:
            ...
            try:
                ...
                for i, remote in enumerate(rmts):
                    ...
                    progress.update(
                        ...
                        i,
                        len_rmts,
                        prefix + "Fetching remote {} of submodule {!r}".format(remote, self.name),
                    )
                    # ===============================
                    if not dry_run:
                ...
                progress.update(
                    ...
                    1,
                    prefix
                    + "Cloning url '{}' to '{}' in submodule {!r}".format(self.url, checkout_module_abspath, self.name),
                )
                if not dry_run:
            ...
            if mrepo is not None and mrepo.head.commit.binsha != binsha:
                ...
                progress.update(
                    ...
                    1,
                    prefix
                    + "Updating working tree at {} for submodule {!r} to revision {}".format(self.path, self.name, hexsha),
                )
    ...
    def remove(
        ...
        if module and self.module_exists():
            ...
            if force:
                ...
                # submodules first.
                mp = self.abspath
                method: None | Callable[[PathLike], None] = None
                if osp.islink(mp):
                    method = os.remove
                elif osp.isdir(mp):
                    method = rmtree
    ...
    def set_parent_commit(self, commit: Commit_ish | str | None, check: bool = True) -> "Submodule":
        ...
        if self.k_modules_file not in pctree:
            raise ValueError("Tree of commit {} did not contain the {} file".format(commit, self.k_modules_file))
        # END handle exceptions
        ...
        if check:
            ...
            if not parser.has_section(sm_section(self.name)):
                ...
                raise ValueError("Submodule at path {!r} did not exist in parent commit {}".format(self.path, commit))
    ...
    def iter_items(
        cls,
        repo: "Repo",
        parent_commit: Commit_ish | str = "HEAD",
        *args: Any,
        **kwargs: Any,
    ) -> Iterator["Submodule"]:
        """
