...
def py_where(program: str, path: PathLike | None = None) -> list[str]:
    ...
    for folder in str(path).split(os.pathsep):
        ...
        if folder:
            ...
            for f in [exe_path] + ["{}{}".format(exe_path, e) for e in winprog_exts]:
                if is_exec(f):
                    progs.append(f)
    return progs
...
def _cygexpath(drive: str | None, path: str) -> str:
    if osp.isabs(path) and not drive:
        ...
        if osp.isabs(p):
            ...
            p = "/proc/cygdrive/{}/{}".format(drive.lower(), p)
...
_cygpath_parsers: tuple[tuple[Pattern[str], Callable, bool], ...] = (
    ...
    (
        re.compile(r"\\\\\?\\UNC\\([^\\]+)\\([^\\]+)(?:\\(.*))?"),
        (lambda server, share, rest_path: "//{}/{}/{}".format(server, share, rest_path.replace("\\", "/"))),
        False,
    ),
...
def decygpath(path: PathLike) -> str:
    ...
    if m:
        ...
        path = "{}:{}".format(drive.upper(), rest_path or "")
...
def get_user_id() -> str:
    ...
    return "{}@{}".format(getpass.getuser(), platform.node())
...
class Actor:
    ...
    def __repr__(self) -> str:
        return '<git.Actor "{} <{}>">'.format(self.name, self.email)
        ...
        while True:
            try:
                ...
                if not osp.isdir(osp.dirname(self._lock_file_path())):
                    msg = "Directory containing the lockfile {!r} was not readable anymore after waiting {:g} seconds".format(
                ...
                if curtime >= maxtime:
                    msg = "Waited {:g} seconds for lock at {!r}".format(
                        maxtime - starttime,
                        self._lock_file_path(),
                    )
                    raise OSError(msg) from e
