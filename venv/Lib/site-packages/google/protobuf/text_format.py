...
class _Parser:
  ...
  def _MergeMessageField(self, tokenizer, message, field):
    ...
    while not tokenizer.TryConsume(end_token):
      if tokenizer.AtEnd():
        raise tokenizer.ParseErrorPreviousToken('Expected "{}".'.format(end_token))
...
class Tokenizer:
  ...
  def _ConsumeSingleByteString(self):
    ...
    if len(text) < 1 or text[0] not in _QUOTES:
      raise self.ParseError('Expected string but found: {!r}'.format(text))

    if len(text) < 2 or text[-1] != text[0]:
      raise self.ParseError('String missing ending quote: {!r}'.format(text))
