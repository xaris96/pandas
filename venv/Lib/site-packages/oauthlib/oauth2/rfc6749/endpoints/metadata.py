    ...
    def validate_metadata(self, array, key, is_required=False, is_list=False, is_url=False, is_issuer=False):
        ...
        if key not in array:
            ...
            if not utils.is_secure_transport(array[key]):
                raise ValueError(f"key {key}: {array[key]} must be an HTTPS URL")
            if "?" in array[key] or "&" in array[key] or "#" in array[key]:
                raise ValueError(f"key {key}: {array[key]} must not contain query or fragment components")
            ...
            for elem in array[key]:
                if not isinstance(elem, str):
                    raise ValueError(f"array {key}: {array[key]} must contains only string (not {elem})")
