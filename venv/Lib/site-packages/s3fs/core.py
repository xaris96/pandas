...
from fsspec.utils import (
    setup_logging as setup_logger,
    tokenize,
)
from s3fs.errors import translate_boto_error
from s3fs.utils import (
    FileExpired,
    ParamKwargsHelper,
    S3BucketRegionCache,
    _get_brange,
...
class S3FileSystem(AsyncFileSystem):
    ...
    async def _mkdir(self, path, acl=False, create_parents=True, **kwargs):
        ...
        if await self._exists(bucket):
            ...
            try:
                ...
                raise ValueError("Bucket create failed {!r}: {}".format(bucket, e))
        else:
    ...
    async def _info(self, path, bucket=None, key=None, refresh=False, version_id=None):
        ...
        try:
            ...
            raise ValueError("Failed to list path {!r}: {}".format(path, e))
    ...
    async def _copy_basic(self, path1, path2, **kwargs):
        ...
        try:
            ...
            raise ValueError("Copy failed ({!r} -> {!r}): {}".format(path1, path2, e)) from e
    ...
    def _upload_chunk(self, final=False):
        ...
        if (
            ...
            def upload_part(part_data: bytes):
                ...
                logger.debug(
                    "Upload chunk {}, {}; {} bytes".format(self, part, len(part_data))
                )
