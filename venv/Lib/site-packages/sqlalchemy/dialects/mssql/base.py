    ...
    def result_processor(self, dialect, coltype):
        def process(value):
            if isinstance(value, datetime.datetime):
                ...
                if not m:
                    raise ValueError(
                        "could not parse {!r} as a date value".format(value)
                    )
    ...
    def result_processor(self, dialect, coltype):
        def process(value):
            if isinstance(value, datetime.datetime):
                ...
                if not m:
                    raise ValueError(
                        "could not parse {!r} as a time value".format(value)
                    )
...
class MSTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_FLOAT(self, type_, **kw):
        ...
        if precision is None:
            ...
            return "FLOAT({precision})".format(precision=precision)
...
class MSSQLCompiler(compiler.SQLCompiler):
    ...
    def visit_extract(self, extract, **kw):
        ...
        return "DATEPART({}, {})".format(field, self.process(extract.expr, **kw))
...
class MSIdentifierPreparer(compiler.IdentifierPreparer):
    ...
    def quote_schema(self, schema, force=None):
        ...
        if dbname:
            result = "{}.{}".format(self.quote(dbname), self.quote(owner))
        elif owner:
