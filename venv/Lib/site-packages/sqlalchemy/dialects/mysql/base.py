...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_on_duplicate_key_update(self, on_duplicate, **kw):
        ...
        for column in (col for col in cols if col.key in on_duplicate_update):
            ...
            clauses.append("{} = {}".format(name_text, value_text))
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        ...
        return "MATCH ({}) AGAINST ({})".format(match_clause, against_clause)
    ...
    def visit_cast(self, cast, **kw):
        ...
        return "CAST({} AS {})".format(self.process(cast.clause, **kw), type_)
    ...
    def limit_clause(self, select, **kw):
        ...
        if limit_clause is None and offset_clause is None:
            ...
            return " \n LIMIT {}".format(self.process(limit_clause, **kw))
...
class MySQLDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_create_index(self, create, **kw):
        ...
        if parser is not None:
            text += " WITH PARSER {}".format(parser)
...
class MySQLTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_NUMERIC(self, type_, **kw):
        if type_.precision is None:
            ...
            return self._extend_numeric(
                type_,
                "NUMERIC({precision})".format(precision=type_.precision),
            )
        else:
            return self._extend_numeric(
                type_,
    ...
    def visit_DECIMAL(self, type_, **kw):
        if type_.precision is None:
            ...
            return self._extend_numeric(
                type_,
                "DECIMAL({precision})".format(precision=type_.precision),
            )
        else:
            return self._extend_numeric(
                type_,
    ...
    def visit_FLOAT(self, type_, **kw):
        if (
            ...
            return self._extend_numeric(
                type_, "FLOAT({}, {})".format(type_.precision, type_.scale)
            )
        elif type_.precision is not None:
            return self._extend_numeric(
                type_, "FLOAT({})".format(type_.precision)
            )
        else:
    ...
    def visit_CHAR(self, type_, **kw):
        if type_.length is not None:
            return self._extend_string(
                type_, {}, "CHAR({length})".format(length=type_.length)
            )
        else:
    ...
    def visit_NVARCHAR(self, type_, **kw):
        ...
        if type_.length is not None:
            return self._extend_string(
                type_,
                {"national": True},
                "VARCHAR({length})".format(length=type_.length),
            )
        else:
            raise exc.CompileError(
    ...
    def visit_NCHAR(self, type_, **kw):
        ...
        if type_.length is not None:
            return self._extend_string(
                type_,
                {"national": True},
                "CHAR({length})".format(length=type_.length),
            )
        else:
